...
export const colors = {
  // Teal-700: WCAG AA contrast (4.5:1+) for text and buttons
  primary: '#0F766E',
...
const getPalette = (mode: PaletteMode) =>
  mode === 'light'
    ...
    : {
        mode: 'dark' as const,
        // Lighter primary for WCAG AA on dark backgrounds (#1A1F26, #242B33)
        primary: {
          main: '#2DD4BF', // teal-400: ~8:1 on dark paper
          light: '#5EEAD4',
          dark: '#0D9488',
          contrastText: 'rgba(0, 0, 0, 0.87)',
        },
...
export function createAppTheme(mode: PaletteMode) {
  ...
  return createTheme({
    ...
    shadows,
    zIndex: {
      // Above bottom nav (1400) so dialogs/modals appear on top
      modal: 1500,
    },
    shape: {
      borderRadius: 12,
